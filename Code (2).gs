...
function _recordCat(p, imgData){
  ...
  // 重複Discord通知防止: image_backup フラグ付き（GET保険）の場合、
  // 直前2分以内に同一cid+noで既にDiscord送信ログがあればスキップ
  if(String(p.image_backup||'')==='1'){
    try{
      const dsh = _sheet('discord', ['ts','type','name','status','msgId','hasImage','detail']);
      const dv  = dsh.getDataRange().getValues();
      const now = new Date().getTime();
      let dupe=false;
      for(let i=dv.length-1;i>=Math.max(1,dv.length-30);i--){
        const rTs = new Date(dv[i][0]).getTime();
        if(isNaN(rTs) || (now - rTs) > 120000) break;
        if(String(dv[i][2]||'')===String(name) && String(dv[i][5]||'')==='yes'){
          dupe=true; break;
        }
      }
      if(dupe) return {ok:true, hasImage:false, skipped:'backup-dedupe'};
    }catch(_){}
  }
...
function _sendDiscord(payload, p, imgData){
  if(!WEBHOOK) return;
  const log = _sheet('discord', ['ts','type','name','status','msgId','hasImage','detail']);
  const rec = function(status,msgId,detail){
    try{ log.appendRow([new Date().toISOString(), payload.thread_name||'', (p&&p.name)||'', status, msgId||'', imgData?'yes':'no', detail||'']); }catch(_){}
  };
  try{
    // ─── ケースA: 画像なし。JSON送信のみ（従来通り）
    if(!imgData){
      const url = WEBHOOK + '?wait=true';
      const res = _sendJSON(url, payload);
      let body={}; try{ body=JSON.parse(res.getContentText()||'{}'); }catch(_){}
      rec(res.getResponseCode(), body.id, body.channel_id||'');
      return;
    }
    // ─── ケースB: 画像あり。二段送信で確実に配信
    // Step 1) 親メッセージを thread_name 付きで作成（画像なし・Embedのみ）
    //         → 新スレッドが生成される
    const parentPayload = Object.assign({}, payload);
    // Embed の image を一旦外す（画像は Step2 で送る）
    if(parentPayload.embeds && parentPayload.embeds.length){
      parentPayload.embeds = parentPayload.embeds.map(function(em){
        var e2=Object.assign({}, em); delete e2.image; return e2;
      });
    }
    const parentUrl = WEBHOOK + '?wait=true';
    const parentRes = UrlFetchApp.fetch(parentUrl, {
      method:'post', contentType:'application/json',
      payload: JSON.stringify(parentPayload), muteHttpExceptions:true
    });
    const pcode = parentRes.getResponseCode();
    let parentBody={}; try{ parentBody=JSON.parse(parentRes.getContentText()||'{}'); }catch(_){}
    const messageId = parentBody.id;
    const channelId = parentBody.channel_id;
    rec(pcode, messageId, 'parent-created');

    if(!messageId){ rec('err','','parent-no-id'); return; }

    // Step 2) 直下スレッドへ multipart で画像添付
    //         Webhook では ?thread_id={message_id} で対象スレッドに投稿できる
    const m = /^data:(image\/[a-zA-Z0-9.+-]+);base64,(.+)$/.exec(imgData);
    if(!m){ rec('err','','bad-dataurl'); return; }
    const mime = m[1] || 'image/png';
    const b64  = m[2];
    const bytes = Utilities.base64Decode(b64);
    const blob  = Utilities.newBlob(bytes, mime, 'ieneko_card.png');

    // スレッド内投稿用の payload_json（画像添付を Embed で指す）
    const threadPayload = {
      content: '🐾 家猫カードはこちら',
      embeds: [{
        image: { url: 'attachment://ieneko_card.png' },
        color: 0x1F3864
      }]
    };
    const threadUrl = WEBHOOK + '?wait=true&thread_id=' + encodeURIComponent(messageId);
    const threadRes = UrlFetchApp.fetch(threadUrl, {
      method:'post',
      payload: {
        'payload_json': JSON.stringify(threadPayload),
        'files[0]': blob
      },
      muteHttpExceptions: true
    });
    let tbody={}; try{ tbody=JSON.parse(threadRes.getContentText()||'{}'); }catch(_){}
    rec(threadRes.getResponseCode(), tbody.id||'', 'thread-image-'+(threadRes.getContentText()||'').slice(0,120));
  }catch(err){
    rec('err','',String(err&&err.message||err));
  }
}
